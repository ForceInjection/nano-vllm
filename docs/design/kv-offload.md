...
- **公平对比 = RECOMPUTE vs SWAP**：二者 cap 相同、`avg_batch` 都 =4.7（并发一致），只差抢占策略。baseline 的 `avg_batch`=32 是"无抢占天花板"——它快是因为 batch 大 8 倍，**不参与 swap 对比**（抢占的固有代价就是并发变小，无法与无抢占等并发）。
- **机制层面 swap 恒赢（与模型大小无关）**：看 `prefill_tok`——RECOMPUTE 重跑被抢占序列的整段 prefill → 9904（baseline 5792 的 1.7×）；SWAP 不重跑 → 7848（≈baseline，多出的是 8 次护栏回退重算）。**SWAP 少做 ~2000 prefill token 的白工。**
- **墙钟仍打平**（130.1 vs 130.6）：0.6B 太小，省下的重算 ≈ PCIe 搬运成本，相互抵消。
- swap 的收益 = 省下的重算（∝ 模型规模 × prompt 长度）− PCIe 搬运（∝ KV 字节/带宽）。小模型打平，大模型（14B/32B，重算长 prefill 极贵而搬运不变）才大胜——这也解释了 vLLM 为何在大模型场景才把 swap 当默认手段。
