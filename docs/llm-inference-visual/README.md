...
具体使用规范与图资产组织见 [CLAUDE.md](../../CLAUDE.md) 的「Documentation conventions → Visual course」一节。
...
课程按推理链路从外到内展开：先打通端到端主干，再逐层拆开数据结构、调度、显存、批构建、注意力，最后把常见优化叠回推理链路上。每课均包含：本课概述（含课时安排与学习目标）、原理铺垫（如适用）、关键代码锚点与嵌入片段、示意图（Mermaid / draw.io）、最小练习与验收要点。
...
以下是最小的本地运行建议，用于把"代码阅读"与"实际运行现象"对齐。nano-vllm 运行需要本地模型权重文件，仓库提供了示例用法。
