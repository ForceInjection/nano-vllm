...
---
layout: default
<!-- 讲解 2.1 为什么 decode 只需送 1 个 token -->
---
...
---
layout: default
<!-- 讲解 2.2 Prefill vs Decode：不同 API 的原因 -->
---
...
---
layout: default
<!-- 讲解 3.1 get_context：上下文来源 -->
---

# 3.1 get_context：上下文来源

<SourceCode file="nanovllm/utils/context.py" lines="5-15" />

```python
from nanovllm.utils.context import get_context

context = get_context()                          # ① 获取线程局部的 Context 单例
```

<div v-click class="mt-3 p-3 bg-green-500/10 border-l-3 border-green-500 rounded-r text-sm">
  <strong>get_context()</strong> 返回 <code>_CONTEXT</code> 全局单例（context.py:L16-L19）。<br/>
...
layout: default
<!-- 讲解 forward 方法完整代码（中）：Prefill 分支 -->
---
...
---
layout: default
<!-- 讲解 forward 方法完整代码（下）：Decode 分支 + 返回 -->
---
...
<div v-click="2" class="mt-3 p-3 bg-green-500/10 border-l-3 border-green-500 rounded-r text-sm">
  ...
  ⑬ <code>block_tables[i][j]</code> = seq i 的第 j 个 block 在 k_cache 中的物理索引。<br/>
  ⑭ 返回注意力输出 o，形状因 prefill/decode 分支而异。
</div>

<!-- 幻灯片 -->
---
layout: default
<!-- 讲解 3.1 store_kvcache：用 slot_mapping 写入 KV cache -->
---
...
---
layout: default
<!-- 讲解 store_kvcache Triton kernel 逐行解读 -->
---
...
if context.is_prefill:
    if context.block_tables is not None:        # prefix cache 命中
        k, v = k_cache, v_cache      # 用缓存的 K/V 替代新计算的
...
<div v-click class="mt-3 p-3 bg-green-500/10 border-l-3 border-green-500 rounded-r text-sm">
  <strong>Prefill 分支总结</strong>：<br/>
  ⑤ <code>is_prefill</code> 由 Scheduler 设置，决定走 prefill 路径。<br/>
  ⑥ 有 <code>block_tables</code> 时走 prefix cache 分支。<br/>
  ⑦ K/V 被替换为 cache 中的历史数据。<br/>
  ⑧ <code>flash_attn_varlen_func</code> 用 cu_seqlens 处理变长序列。
</div>
...
<div v-click class="mt-3 p-3 bg-green-500/10 border-l-3 border-green-500 rounded-r text-sm">
  ...
  <ul class="mt-1 space-y-1">
    <li>decode 的 <code>q.unsqueeze(1)</code> 增加 seqlen=1 维度（形状 [bs, 1, nh, hd]）</li>
    <li>decode 的 K/V 是 <code>k_cache, v_cache</code>（历史缓存），不是新计算的 k, v</li>
    <li>prefill 用 <code>cu_seqlens_q/k</code> — 变长序列的累积边界数组</li>
    <li>decode 用 <code>cache_seqlens</code> — 每 seq 一个整数，表示 cache 中已有 KV 长度</li>
  </ul>
</div>
...
<!-- 讲解 slot_mapping → store_kvcache → block_tables 的数据流 -->
---
...
<!-- 讲解 §3-4：prefix cache 触发条件 + 真实 Context 类 -->
---
