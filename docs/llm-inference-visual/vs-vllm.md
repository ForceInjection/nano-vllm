...
nano-vllm 约为 1,200 行 Python，真实 vLLM 约为 200,000+ 行。本文标注二者的关键差异，区分"教学简化"和"真正未实现的功能"，帮助读者建立从 nano 到 real 的升级路径。
...
| Prefix Caching | xxhash 链式哈希, hash_to_block_id 全局字典 | 相同方案, 外加 LRU eviction of hash entries | L04 hash_blocks() |
...
4. **块管理器**：`vllm/core/block_manager_v2.py` — 对应 nano 的 `nanovllm/engine/block_manager.py`
