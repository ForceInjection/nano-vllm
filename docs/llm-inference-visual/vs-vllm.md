...
### 2.3 KV Cache Offloading —— ✅ 本项目已实现（简化版 swap）

> 本条原为"缺失能力"，现已在本项目补齐，属于"教学简化实现"（概念保留、实现精简）；保留在此以体现"从缺失到实现"的升级脉络。设计与验证详见 [`../design/kv-offload.md`](../design/kv-offload.md)。

当 GPU 显存不够时，真实 vLLM 可以将不常用的 KV cache block 交换到 CPU 内存或 NVMe 磁盘。

- nano 里（原）：block 不足时只能 preempt（丢弃 KV cache，下一轮 prefill 重算）
- nano 里（现）：`cpu_offload_gb>0` 时启用 **swap 抢占**——引入 `swapped` 第三队列，被抢占序列的 KV 块搬到 CPU pinned 镜像（swap_out），空出后搬回 GPU（swap_in）续跑；CPU 也满或序列含共享块时回退 RECOMPUTE
- vLLM 里：swap out → swap in（含 NVMe 磁盘层）
- **与 vLLM 的差异（教学简化）**：只有 CPU 层、无 NVMe；CPU 块不参与前缀缓存；同步 `copy_`、无 CUDA stream 重叠；reactive-only（仅抢占时触发，非主动分层卸载）
- **实测**（RTX 3090 / Qwen3-0.6B）：小模型上 SWAP ≈ RECOMPUTE（PCIe 搬运成本 ≈ 重算成本），收益随模型规模/prompt 长度增长——见设计文档 §10.E
